...
For the equation of motion considering the inserted values we get:
...
\begin{equation}\label{eq:3.6.4}
    \begin{split}
        \frac{{\mathrm{thetad}}^2\,\sin\left(\phi \right)}{128}+\frac{981\,\sqrt{3}\,\sin\left(\phi \right)}{3200}+\frac{\sqrt{3}\,{\mathrm{thetad}}^2\,\sin\left(\phi \right)}{512}-\frac{{\mathrm{thetad}}^2\,\cos\left(\phi \right)\,\sin\left(\phi \right)}{256} = 0
    \end{split}
\end{equation}
...
\begin{equation}
    \begin{split}
        \frac{\mathrm{thetadd}\,\left(3\,\cos\left(\phi \right)-\frac{3\,{\cos\left(\phi \right)}^2}{4}+\frac{21\,\sqrt{3}}{5}+\frac{3\,\sqrt{3}\,\cos\left(\phi \right)}{4}+\frac{251}{20}\right)}{192} = 0
    \end{split}
\end{equation}
...
This makes sense as $\phi = n*\pi$ describes states where the square plate stays vertical. As we are not considering aerial drag it is imaginable that this is an equilibrium.

The same holds for a free rotation around A as can bee seen in equation \ref{eq:3.6.4}.

For the equation w.r.t. $\theta$ we get a more complicated result.